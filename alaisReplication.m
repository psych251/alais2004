...
%
%   %%% IMPORTANT %%%
%   Running this program requires mgl libraries (gru.stanford.edu/mgl).
%   
%
%
%
%      usage: myscreen = alaisReplication()
%         by: josh wilson
%       date: October 2021
...
%            alaisReplication('visual=1');
%            alaisReplication('auditory=1');
%            alaisReplication('bimodal=1');
%
...
%
%            alaisReplication('doGammaTest=1');
%
%            To run with a noise background with SNR of 2.5 that updates 4.5 every sec
%
%            alaisReplication('SNR=2.5','backgroundFreq=4.5');
%
...
%
%            alaisReplication('visual=1','useStaircase=1');
%
...
%
%            alaisReplication('visual=1','useStaircase=1','restartStaircase=1');
%
%            By default there will be 40 trials per staircase, you can change this with:
%
%            alaisReplication('visual=1','useStaircase=1','nStaircaseTrials=50');
%
%            TO run w/out eye tracker calibration
%
%            alaisReplication('doEyecalib=0');
%
%
function myscreen = alaisReplication(varargin)
...
% get arguments
bimodal = 0;
getArgs(varargin,{'width=1','doEyecalib=0','visual=0','auditory=0','bimodal=1','dispPlots=1','auditoryTrain=0','visualTrain=0','tenbit=1','doGammaTest=0','stimulusContrast=1','SNR=1.3','doTestSNR=0','backgroundFreq=2.05','doTestStimSize=0','maxSNR=1.3','useStaircase=0','nStaircaseTrials=40','restartStaircase=0'},'verbose=1');
...
task{1}{1}.random = 1;
task{1}{1}.parameter.posDiff = [0]; 
if stimulus.task == 3
    %bimodal displacement
  task{1}{1}.parameter.displacement = [-10];
end
...
if doTestStimSize
  for iWidth = 1:length(stimulus.width)
    ...
    mglFlush;
    disp(sprintf('(alaisReplication) Screen width: %0.1f Screen height: %0.1f Gaussian width: %0.1f',myscreen.imageWidth, myscreen.imageHeight, stimulus.width(iWidth)));
    askuser('Ok');
  end
  return
end
...
if doTestSNR
  ...
  for iWidth = 1:length(stimulus.width)
    while iSNR <= length(stimulus.SNR);
      ...
      if ~askuser(sprintf('(alaisReplication) SNR=%0.1f Display again',stimulus.SNR(iSNR)),-1)
...
if stimulus.disp
  ...
  if any(~isnan(e{end}.response))
    ...
    disp(sprintf('(alaisReplication) No subject responses to plot psychometric function with.'));
  end
end
...
if stimulus.task ~= 2 
  ...
  if isinf(stimulus.SNR)
	...
	disp(sprintf('!!! (alaisReplication) Stimulus is being displayed on a different noisy background then what is currently being presented. You should adjust the backgroundFreq until this no longer happens'));
      end
    elseif task.thistrial.thisseg == stimulus.interval(2)
...
if ~task.thistrial.gotResponse
    ...
    (task.thistrial.centerWhich == 2 && ((task.thistrial.diff > 0 && task.thistrial.whichButton == 2) || ...
	  ...
	  stimulus.fixColor = stimulus.colors.green;%[0 1 0];
        ...
        if ~stimulus.bimodal
            disp(sprintf('(alaisReplication) Trial %i: %0.2f %c correct centerInt %i resp %i', ...
             task.trialnum, task.thistrial.diff, task.thistrial.left, task.thistrial.centerWhich, task.thistrial.whichButton))
        else
            disp(sprintf('(alaisReplication) Trial %i: %0.2f %i %c correct centerInt %i resp %i', ...
	  ...
	  stimulus.fixColor = stimulus.colors.red;%[1 0 0];
        ...
        if ~stimulus.bimodal
            disp(sprintf('(alaisReplication) Trial %i: %0.2f %c incorrect centerInt %i resp %i', ...
             task.trialnum, task.thistrial.diff, task.thistrial.left, task.thistrial.centerWhich, task.thistrial.whichButton))
        else
            disp(sprintf('(alaisReplication) Trial %i: %0.2f %i %c incorrect centerInt %i resp %i', ...
             task.trialnum, task.thistrial.diff, task.thistrial.displacement, task.thistrial.left, task.thistrial.centerWhich, task.thistrial.whichButton))
        end
    end
...
if ~stimulus.restartStaircase && stimulus.visual && ~stimulus.visualTrain && ~isempty(stimulus.visualStaircase)
  disp(sprintf('(alaisReplication) Setting staircase to threshold from previous run'));
...
if stimulus.tenbit
  ...
  if ~isfield(myscreen,'gammaTable')
    ...
    disp(sprintf('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'));
    disp(sprintf('(alaisReplication:initGratings) No gamma table found in myscreen. Contrast'));
...
disp(sprintf('(alaisReplication:testGammaTable) Top row should be reserved colors'));
...
disp(sprintf('(alaisReplication:testGammaTable) Bottom row should be stimulus colors'));
mglFlush;
...
mlrSmartfig('alaisReplication','reuse');clf;
...
if any(SNR > maxSNR)
  disp(sprintf('(alaisReplication:setBackgroundNoise) max SNR is: %0.1f and an SNR of %0.1f was called for. Need to set the maxSNR setting higher - but note that this will change the maximum luminance of the noise background',maxSNR,max(SNR)))
  stimulus = [];
  return
end
...
disppercent(-inf,'(alaisReplication:setBackgroundNoise) Precomputing background noise images');
